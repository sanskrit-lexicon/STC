"""
Markup fixer + audit for stc.txt (STC).
Counterpart of PWG#175, PWK#113, and siblings.

STC-specific notes: 0 whitespace hits. 9,826 adj </ab><ab>. 7 non-standard <ab n> (French).

Inputs: ../../../csl-orig/v02/stc/stc.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "stc" / "stc.txt", HERE / "stc.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "stc_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(ab|F|lang)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for stc.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"STC markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against stc.txt.\n")
        f.write(f"Paired tags: <ab>, <F>, <lang>\n")
        f.write(f"Notes: 0 whitespace hits. 9,826 adj </ab><ab>. 7 non-standard <ab n> (French).\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <ab>, <F>, <lang>\n\n")
